...
def simple_environ(
    ...
    return {f'{prefix}key': env_value, 'a': 'b'}


def same_environ() -> dict[str, str]:
    """Returns dict with example environment with given prefix and value."""
    return {'NORMAL_KEY': 'test'}


def multiple_variables_with_prefix() -> dict[str, str]:
...
class TestDump:
    ...
    def test_with_multiple_prefixes(
        self,
        monkeypatch: pytest.MonkeyPatch,
    ) -> None:
...
class TestDumpRegression:
    ...
    def test_multiple_vars_with_prefix(
        self,
        monkeypatch: pytest.MonkeyPatch,
        env_file: str,
